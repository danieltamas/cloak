...
**Code signing — important limitation:** The data protection keychain requires the binary to carry the `keychain-access-groups` entitlement, which is a *restricted* entitlement. A freely-distributed **ad-hoc** binary that claims it is **SIGKILL'd by AMFI at launch** (`Killed: 9`) on end-user machines — confirmed by bricking v0.4.2. Honoring a restricted entitlement needs a real Apple Developer ID signature (paid, with provisioning), which cloak does not have. **Therefore the biometric (`cloak-bio`) keychain path cannot function on distributed builds** — `SecItemAdd` returns `errSecMissingEntitlement` (-34018) and `keychain/mod.rs` silently falls back to the legacy login keychain. The `cloak-bio` code is kept as a no-op-until-properly-signed path; the actual Touch ID UX comes from the `cloak-touchid` auth gate (LAContext). _(The original spec's claim that "ad-hoc signing is sufficient" was wrong on both counts: ad-hoc lacks the entitlement, and adding the entitlement to an ad-hoc binary makes it unloadable.)_
